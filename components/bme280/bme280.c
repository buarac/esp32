...
esp_err_t bme280_chip_id_get(bme280_t* bme) {
    ...
    esp_err_t ret = i2c_device_read_reg_uint8(&bme->device, BME280_REG_CHIP_ID, &bme->chip_id);
    if ( ret != ESP_OK ) {
        bme->chip_id = 0;
    }
    return ret;
...
esp_err_t bme280_cal_humi_data_get(bme280_t* bme) {
    ...
    i2c_device_read_reg_uint8 (dev, 0xa1, &bme->dig_h1);
    i2c_device_read_reg_int16 (dev, 0xe1, &bme->dig_h2);
    i2c_device_read_reg_uint8 (dev, 0xe3, &bme->dig_h3);
    i2c_device_read(dev, &reg, 1, raw, 3);
    bme->dig_h4 = (int16_t) ( (raw[0] <<4 ) | (raw[1] & 0x0f));
    //bme->dig_h5 = (int16_t) ( );
    i2c_device_read_reg_int8 ( dev, 0xe7, &bme->dig_h6);
...
int32_t bme280_compensate_temperature(bme280_t *bme, int32_t temp, int32_t *fine_temp) {
    ...
    *fine_temp = var1 + var2;
    ...
    ESP_LOGD(TAG, "calculation detail");
